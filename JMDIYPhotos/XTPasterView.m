...
- (void)remove
{
    ...
    if ([_delegate respondsToSelector:@selector(removePaster:)]) {
        [_delegate removePaster:self.pasterID] ;
    }else{
        NSDictionary *dic = [NSDictionary dictionaryWithObject:self forKey:@"paster"];
        [[NSNotificationCenter defaultCenter] postNotificationName:ACTOYSREMOVEPASTER object:nil userInfo:dic];
    }
}
...
//    NSLog(@"%d贴纸中心点%@",self.pasterID,NSStringFromCGPoint(self.center));
//    [self.centerPointDict setObject:NSStringFromCGPoint(self.center) forKey:[NSString stringWithFormat:@"%d",self.pasterID]];
//    NSLog(@"中心点字典%@",_centerPointDict);
//    [self.centerPointArr addObject:_centerPointDict];
//    NSLog(@"中心点数组%@",_centerPointArr);
    touchStart = touch;
}
